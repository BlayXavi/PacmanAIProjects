...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(self.iterations):
            iterationValues = self.values.copy() #to not modify the dictionary while iterating throught it
            for state in states:
                actions = self.mdp.getPossibleActions(state)
                maxStateValue = float('-inf') #optimum value at current state
    ...
    def computeQValueFromValues(self, state, action):
        ...
        qValue = 0.0
        for trans in transStatesAndProbs:
            nextState = trans[0]
            prob = trans[1]
            
            r = self.mdp.getReward(state, action, nextState)
            vNext = self.discount * self.getValue(nextState)
            qValue += prob * (r + vNext)
